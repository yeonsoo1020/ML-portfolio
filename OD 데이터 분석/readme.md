...
## 🔎 분석 방법
1. **전통시장별 입지 데이터 구축**  
   - 유동 인구, 교통 접근성, 시장 인프라, 관심도 등의 변수를 추출하여 데이터셋 구성
2. **영업률 예측 모델 개발**  
   - `RandomForestRegressor`, `XGBRegressor` 등의 모델을 활용하여 전통시장의 영업률 예측
   - 변수 중요도 분석 및 Feature Selection 진행
3. **최적 청년몰 입지 선정**  
   - 입지 평가 기준: 영업률, 점포 수, 교통 접근성, 시설 인프라 등 고려  
   - 데이터 기반으로 부산 내 최적 전통시장 도출 (최종 선정: `초량전통시장`)
4. **고객 페르소나 분석 및 클러스터링**  
   - K-Means 클러스터링을 활용하여 고객군 유형화  
   - 고객군 특성에 맞춘 맞춤형 운영 전략 제시
